...
WordSquad uses a responsive CSS Grid-based layout system that adapts to different viewport sizes:

**Layout Modes:**
- **Light Mode** (≤600px): Mobile-first single-column layout
  - Side panels hidden by default
  - Panels appear as fixed overlay modals when activated
...
- **Medium Mode** (601px-900px): Tablet and small desktop layout
  - Single-column grid layout with center panel
  - Panels appear as centered overlays when opened
  - History, definition, and chat panels overlay the board when active
  - Panels scale appropriately with viewport size

- **Full Mode** (>900px): Large desktop layout
  - Full CSS Grid layout with dedicated grid areas for panels
  - Grid template areas: history | stamp | center | definition | chat
  - Panels positioned using CSS Grid with `grid-area` assignments
  - Automatic spacing and positioning handled by grid gap and template columns
  - Optimal panel width (280px) maintained within grid areas

**Panel System:**
- **History Panel** (`#historyBox`): Shows guess history, uses `grid-area: history`
- **Definition Panel** (`#definitionBox`): Shows word definitions, uses `grid-area: definition`
- **Chat Panel** (`#chatBox`): Real-time chat functionality, uses `grid-area: chat`
- All panels support smooth transitions and maintain proper z-index hierarchy
- Panel visibility controlled exclusively via CSS classes (`history-open`, `definition-open`, `chat-open`)
- Display toggling uses `display: none` and `display: flex/block` in media queries

**Technical Requirements:**
- Pure CSS Grid layout - no JavaScript positioning logic required
- CSS-based layout switching using `body[data-mode]` attributes set by JavaScript
- Grid template columns and areas defined in responsive media queries
- Proper transform origins for animation effects
